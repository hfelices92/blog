<x-layouts.admin>
    <div class="mb-4 flex justify-between items-center">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="#">{{ __('Create Role') }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>


    <form class="space-y-4  px-6 rounded-lg shadow-lg py-6" method="POST" action="{{ route('admin.roles.store') }}">
        @csrf
                ...
                @foreach ($permissions as $permission)
                    <li class="mb-2">
                        <label class="inline-flex items-center">
                            <input @checked(in_array($permission->id, old('permissions', []))) type="checkbox" name="permissions[]"
                                value="{{ $permission->id }}" class="form-checkbox h-5 w-5 text-blue-600">
