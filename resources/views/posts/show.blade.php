        ...
        <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
            <article
                ...
                <header class="mb-4 lg:mb-6 not-format">
                    <address class="flex items-center mb-6 not-italic">
                        <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                            <flux:avatar circle class="mr-4 p-8 text-xl rounded-full bg-green-400"
                                :name="$post->user->name" />
                            <div>
                                <a href="#" rel="author"
                                    class="text-xl font-bold text-gray-900 dark:text-white">{{ $post->user->name }}</a>
                                <p class="text-base text-gray-500 dark:text-gray-400">Web Developer</p>
                                <p class="text-base text-gray-500 dark:text-gray-400">
                                    <time datetime="{{ $post->published_at->toIso8601String() }}"
                                        title="{{ $post->published_at->format('F jS, Y') }}">{{ $post->published_at->format('M. d, Y') }}
                                    </time>
                                </p>
                            </div>
                        </div>
                ...
                <div>
                    ...
                    <figure>
                        <img src="{{ $post->image }}" alt="Image  of {{ $post->title }}" class="w-full rounded-lg"
                        alt="">
